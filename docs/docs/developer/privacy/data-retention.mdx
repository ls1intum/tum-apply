...
For the self-service export flow, see [Data Export](./data-export).
...
:::note
Both methods use the same property key (`user.retention.cron`) but different fallback defaults in code.
:::
