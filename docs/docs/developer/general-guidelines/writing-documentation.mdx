...
## 5. Embed Videos with the VideoEmbed Component

For screen recordings and other embedded videos, use the custom `<VideoEmbed>` component instead of writing an `<iframe>` by hand. This keeps dimensions, borders, and captions consistent across the docs and produces semantically correct `<figure>` / `<figcaption>` markup.
...
| Prop | Default | Notes |
|---|---|---|
| `width` | `900` | Standard width for docs videos |
| `height` | `520` | Standard height (~16:9 at 900px) |
| `caption` | _(none)_ | Optional; rendered as `<figcaption>` below the video |

Override `width` / `height` only when a specific recording needs it — sticking to the defaults keeps the docs visually consistent.
