...
Best practices for database design, JPA patterns, SQL queries, and performance optimization in DocApply.
...
```java
@Query("""
    SELECT new docapply.dto.JobPostingPreviewDTO(jp.id, jp.title, jp.deadline)
    FROM JobPosting jp
