...
- **`tumapplypreset.ts`**, **`custompreset.ts`**, **`aquabloom.ts`** — each defines its own **color palette** (primary, secondary, etc.). Changes to colors in one preset do **not** propagate to the others.
...
For PrimeNG component overrides using `styleClass` vs `prime-ng-overrides.scss`, see the [Client Styling](../general/client-styling-guidelines#styling-primeng-components) page.
...
- See [Client Styling Guidelines](../general/client-styling-guidelines) for styling best practices
- See [Client Guidelines](../general/angular-guidelines) for Angular and TypeScript patterns
