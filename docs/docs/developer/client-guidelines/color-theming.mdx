---
sidebar_label: Color Theming
---

# Color Theming

DocApply uses a comprehensive theming system built on **PrimeNG Themes** and **Tailwind CSS v4**. The system supports multiple themes with automatic dark mode detection and seamless switching.
...
- **`docapplypreset.ts`** — Main DocApply theme (light + dark modes)
...
@custom-variant dark (&:where(.docapply-dark-mode, .docapply-dark-mode *));
```
...
The `dark:` variant works with the `.docapply-dark-mode` class applied to `<html>`.
...
- **`shared-theme-config.ts`** — primitives (border radius), component overrides, and shared color schemes. Changes here apply to **all themes automatically**.
- **`docapplypreset.ts`**, **`custompreset.ts`**, **`aquabloom.ts`** — each defines its own **color palette** (primary, secondary, etc.). Changes to colors in one preset do **not** propagate to the others.

:::warning
If you add or change a color in one preset (e.g., a new primary shade in `docapplypreset.ts`), you must add the equivalent in `custompreset.ts` and `aquabloom.ts` as well. Otherwise, those themes will fall back to PrimeNG defaults and look inconsistent.
:::
...
- `src/main/webapp/content/theming/docapplypreset.ts`
...
For PrimeNG component overrides using `class` vs `prime-ng-overrides.scss`, see the [Client Styling](./client-styling#styling-primeng-components) page.
...
- CSS class application (`.docapply-dark-mode`, `.docapply-blossom`, `.docapply-aquabloom`)
...
- See [Client Styling](./client-styling) for styling best practices
- See [Client Development](./client-development) for Angular and TypeScript patterns
