...
- Never committed or deployed – listed in `.gitignore` by default.
- You can find an example in `.env.example` under the root of
  the project.
...
2. Add the same variable with an **example value** (do not expose secrets) to `.env.example` and
   explain what this
   variable is for.
...
:::info
A shared `.env.example` file exists in the repository and documents all required variables.
