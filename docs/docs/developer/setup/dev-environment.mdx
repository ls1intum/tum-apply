...
:::info[Platform Notes]
- **macOS**: Requires MySQL CLI to be installed via Homebrew (`brew install mysql-client`) and available in your `PATH`.
- **Linux**: Requires `mysql` CLI to be installed (`sudo apt install mysql-client` or equivalent).
- **Windows**: Use **Git Bash** to run this script. Make sure the MySQL CLI is installed (via MySQL Installer) and the `bin` folder is added to your `PATH`.
:::
...
If you want to use the AI features on the website then you need a local LLM instance. Follow the instructions in the [Local LLM Setup Guide](./spring-ai) to get started.
