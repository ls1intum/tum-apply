...
This document provides an overview of the authentication setup and login flow in **DocApply**, using **Keycloak** and
OAuth 2.0 / OIDC.

## 🔗 Overview

DocApply uses **Keycloak** as an identity provider. The client authenticates via OAuth 2.0 and obtains a JWT token. This
...
- A **public client** called `docapply-client`
...
:::info
The user's **role is not synced from Keycloak**, but instead assigned and managed inside the DocApply **database**.
:::

## ✅ Summary

- Keycloak manages authentication, but not roles.
- User data is stored and managed in the DocApply database.
