...
| External Realm | [http://localhost:9080/realms/external](http://localhost:9080/realms/external) | External login realm endpoint |          |          |

- Open the Admin Console at `http://localhost:9080/admin` and log in with username `admin` and password `admin`.
- Navigate to the `tumidpldap` and `external` realms and verify that the test users listed below are present. (If you
...
`docker/local-setup/realm-config/external-realm.json` during startup.
