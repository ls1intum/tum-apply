...
the DocApply project. It handles user authentication, identity federation, and token issuance, ensuring secure
...
DocApply uses **Keycloak** as the central **identity provider and authentication system**, including support for
identity federation.
...
**DocApply server**.
User roles are assigned and managed within the DocApply database upon first login.
...
| User creation on first login         | DocApply server  |
