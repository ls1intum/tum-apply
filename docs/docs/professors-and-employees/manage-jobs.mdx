...
import myPositionsImg from './images/manage-jobs/my-positions.png';
import jobProfessor1 from './images/manage-jobs/job-detail-professor-1.png';
import jobProfessor2 from './images/manage-jobs/job-detail-professor-2.png';
...
- Displays all positions in a **structured table format** containing: Supervising Professor, Position Title, Status, Start Date, and Last Modified timestamp
- Features **dynamic action buttons** (View, Edit, Delete, Close) that adapt based on the current position state
- Shows **visual status indicators** with color-coded badges for instant assessment of position states
- Includes **search, filtering (by Status), and sort** capabilities to easily find specific positions

<Image src={myPositionsImg} alt="My Positions Dashboard" size={ImageSize.large} caption="My Positions Dashboard"/>
...
- **View The Position (👁️)**: Access complete position details and current status (available for all positions regardless of state)
- **Edit**: Modify position specifications and details. *Note: Editing a Published position will change its status back to "Draft", temporarily hiding it from applicants until republished.*
- **Delete**: Permanently remove positions from the system (available for Draft positions only)
- **Close**: End the application acceptance period. This will automatically reject all unreviewed applications and notify all candidates (available for Published positions only)
...
<Image src={jobProfessor1} alt="Job Details Page Professor 1" size={ImageSize.large}/>
<Image src={jobProfessor2} alt="Job Details Page Professor 2" size={ImageSize.large} caption="Job Detail Page"/>
...
4. **Access quick actions** directly via the Actions menu (⋮):
- Click **"View"** (eye icon) to see detailed position information
- Use **"Edit"** for positions needing updates (Warning: editing a published job reverts it to draft)
- Select **"Delete"** to permanently remove unwanted draft positions
- Choose **"Close"** to shut a published position and reject pending applications

**Video**:
<VideoEmbed
  src="https://live.rbg.tum.de/w/artemisintro/61932?video_only=1&t=0"
  title="My Positions Dashboard Overview"
  caption="Navigating the My Positions dashboard"
/>
...
**Video**:
<VideoEmbed
  src="https://live.rbg.tum.de/w/artemisintro/61933?video_only=1&t=0"
  title="Viewing Job Details"
  caption="Reviewing detailed position information and management actions"
/>
...
**Video**:
<VideoEmbed
  src="https://live.rbg.tum.de/w/artemisintro/61936?video_only=1&t=0"
  title="Editing and Updating Positions"
  caption="Editing an existing position through the multi-step form"
/>
