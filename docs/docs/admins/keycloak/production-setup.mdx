...
This documentation describes the setup and management of the **Keycloak Production instance** used for DocApply. It
...
The Keycloak Production instance is a critical component of the DocApply authentication infrastructure. It is managed
and maintained by the AET Admins and serves both deployed test and production servers.

- **Access URL:** `https://keycloak.aet.cit.tum.de/realms/external-login/`
- **Managed by:** AET Admins

:::danger[Data Persistence]
All data stored in this instance is persistent and must **never** be deleted.
:::
...
Users accessing the production instance can only interact with the `external-login` realm. They do **not** have access
...
- The instance is used for both deployed test and production DocApply servers.
...
the [Keycloak Development Setup](/developer/authentication-and-authorization/keycloak/development-setup).
