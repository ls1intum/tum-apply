...
There are three realms in total for local development:

1. **`master`**
2. **`tumidpldap`**
3. **`external`**

The `tumidpldap` realm is used for TUM login and passkey flows. The `external` realm is used for external/social login
...
- Development includes the `master`, `tumidpldap`, and `external` realms.
