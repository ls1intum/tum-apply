---
sidebar_label: Login
---

import VideoEmbed from "../../src/components/VideoEmbed/VideoEmbed";

# Login

You can browse all open job postings on DocApply without logging in to your account. An account is only necessary if
you want to apply for positions or save and view your personal information and application(s).

## How to Log In

Applicants can log in to DocApply using the following methods:

- **Email and Code:** Enter your email address. You will receive a verification code by email. Enter the code to log in.
No password is required for this method.

<VideoEmbed
  src="https://live.rbg.tum.de/w/artemisintro/67994?video_only=1&t=0"
  title="Login with Email and Code"
  caption="Logging in with email and a verification code"
/>

- **Email and Password:** If you have set a password during registration, you can log in with your email address and
password.

<VideoEmbed
  src="https://live.rbg.tum.de/w/artemisintro/67996?video_only=1&t=0"
  title="Login with Email and Password"
  caption="Logging in with email and password"
/>

- **Google or Apple:** Click the respective button. You will be redirected to the selected provider to
authenticate, and then returned to DocApply, logged in
automatically.

## Important Notes

If you already have an account but are not logged in while applying, you will be asked to enter your email address when
starting an application. You will then receive a verification code by email. After entering the code, you will be
automatically logged in to your existing account.
