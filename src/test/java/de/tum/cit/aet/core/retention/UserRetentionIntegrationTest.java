...
class UserRetentionIntegrationTest {
    ...
    void shouldDeleteApplicantAndAllAssociatedData() throws Exception {
        ...
        User applicantUser = UserTestData.newUserAll(UUID.randomUUID(), "applicant@test.local", "App", "User");
        ...
        UserSetting setting = new UserSetting(savedApplicantUser, "theme", "dark");
        userSettingRepository.save(setting);

        EmailSetting emailSetting = new EmailSetting();
        emailSetting.setUser(savedApplicantUser);
        ...
        profileImage.setUrl("/images/profile.png");
        profileImage.setMimeType("image/png");
        profileImage.setSizeBytes(123L);
        profileImage.setUploadedBy(savedApplicantUser);
        profileImage = imageRepository.save(profileImage);

        userRetentionService.processUserIdsList(List.of(savedApplicantUser.getUserId()), LocalDateTime.now(), false);

        assertThat(userRepository.existsById(savedApplicantUser.getUserId())).isFalse();
        assertThat(applicantRepository.findById(savedApplicantUser.getUserId())).isEmpty();
        assertThat(applicationRepository.findAllByApplicantId(savedApplicantUser.getUserId())).isEmpty();
        assertThat(applicationReviewRepository.findById(review.getApplicationReviewId())).isEmpty();
        assertThat(ratingRepository.findById(rating.getRatingId())).isEmpty();
        assertThat(internalCommentRepository.findById(comment.getInternalCommentId())).isEmpty();
        assertThat(intervieweeRepository.findById(interviewee.getId())).isEmpty();
        assertThat(interviewSlotRepository.findById(slot.getId())).isEmpty();
        assertThat(documentDictionaryRepository.findAllByApplicationApplicationId(application.getApplicationId())).isEmpty();
        assertThat(documentRepository.findAll()).noneMatch(d -> d.getUploadedBy().getUserId().equals(savedApplicantUser.getUserId()));
        assertThat(userSettingRepository.findAllByIdUserId(savedApplicantUser.getUserId())).isEmpty();
        assertThat(emailSettingRepository.findAllByUser(savedApplicantUser)).isEmpty();
        assertThat(imageRepository.findById(profileImage.getImageId())).isEmpty();
        assertThat(userResearchGroupRoleRepository.existsByUserUserId(savedApplicantUser.getUserId())).isFalse();
    }
    ...
    void shouldAnonymizeProfessorDataAndDeleteUser() {
        ...
        userRetentionService.processUserIdsList(List.of(professor.getUserId()), LocalDateTime.now(), false);
        ...
        assertThat(userRepository.existsById(professor.getUserId())).isFalse();
    }
    ...
    void shouldNotChangeAnythingOnDryRun() {
        ...
        userRetentionService.processUserIdsList(List.of(savedApplicantUser.getUserId()), LocalDateTime.now(), true);

        assertThat(userRepository.existsById(savedApplicantUser.getUserId())).isTrue();
        assertThat(applicationRepository.findById(application.getApplicationId())).isPresent();
        assertThat(applicantRepository.findById(savedApplicantUser.getUserId())).isPresent();
    }
    ...
    void shouldSkipUnknownUser() {
        ...
        userRetentionService.processUserIdsList(List.of(unknown.getUserId()), LocalDateTime.now(), false);

        assertThat(userRepository.existsById(unknown.getUserId())).isTrue();
    }
    ...
    void shouldSkipAdminUserAndKeepData() {
        ...
        userRetentionService.processUserIdsList(List.of(admin.getUserId()), LocalDateTime.now(), false);

        assertThat(userRepository.existsById(admin.getUserId())).isTrue();
        assertThat(userSettingRepository.findAllByIdUserId(admin.getUserId())).isNotEmpty();
        ...
        assertThat(userResearchGroupRoleRepository.existsByUserUserId(admin.getUserId())).isTrue();
    }
    ...
    void shouldKeepUserWithoutRolesAndResearchGroup() {
        ...
        userRetentionService.processUserIdsList(List.of(user.getUserId()), LocalDateTime.now(), false);

        assertThat(userRepository.existsById(user.getUserId())).isTrue();
        assertThat(userSettingRepository.findAllByIdUserId(user.getUserId())).isNotEmpty();
    ...
    void shouldProcessMixedBatchAndBeIdempotent() {
        ...
        List<UUID> ids = List.of(savedApplicantUser.getUserId(), professor.getUserId(), unknown.getUserId());
