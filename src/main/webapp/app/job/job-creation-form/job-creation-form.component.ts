...
export class JobCreationFormComponent {
  ...
  /** Holds a pending translation request when a translation is already in-flight */
  private pendingTranslation: { lang: Language; text: string } | null = null;
  ...
  private async translateAndStoreOtherLanguage(currentLang: Language, currentText: string): Promise<void> {
    ...
    // If a translation is already running, coalesce: remember the latest request and return.
    if (this.isTranslating()) {
      this.pendingTranslation = { lang: currentLang, text };
      return;
    }
    ...
    try {
      ...
      // If a newer translation request arrived while we were translating, trigger it now (if it's meaningfully different).
      const pending = this.pendingTranslation;
      this.pendingTranslation = null;
      if (pending) {
        const baselineAfter = pending.lang === 'en' ? this.jobDescriptionEN() : this.jobDescriptionDE();
        if (pending.text.trim() !== baselineAfter) {
          // fire-and-forget the pending translation
          void this.translateAndStoreOtherLanguage(pending.lang, pending.text);
        }
      }
    }
  }
