...
public interface JobRepository extends TumApplyJpaRepository<Job, UUID> {
    /**
     * Finds all jobs created by a specific professor, filtered optionally by title and job state.
    ...
    /**
     * Finds all available job postings with optional filtering and custom sorting by professor name.
    ...
    /**
     * Finds all available job postings with optional filtering options. Sorting is delegated to Spring's Pageable,
     * so this method does not support custom sorting by computed fields like professor name.
     *
     * @param state the job state (typically PUBLISHED)
     * @param title a partial match filter for job title (nullable)
     * @param fieldOfStudies a partial match filter for field of studies (nullable)
